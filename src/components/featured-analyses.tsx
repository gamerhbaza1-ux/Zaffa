...
function FeaturedAnalysisCard({ analysis }: { analysis: { title: string; purchasedCount: number; totalCount: number }}) {
    return (
...
export function FeaturedAnalyses() {
    ...
    const featuredAnalyses = useMemo(() => {
    ...
    return (
        <div className="space-y-6 pt-4">
            {featuredAnalyses.map(analysis => (
                <FeaturedAnalysisCard key={analysis.id} analysis={analysis} />
            ))}
        </div>
    );
}
